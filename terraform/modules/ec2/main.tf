...
resource "aws_launch_template" "app_server" {
  ...
  block_device_mappings {
    ...
    ebs {
      ...
      delete_on_termination = true
      encrypted             = true
      # kms_key_id            = aws_kms_key.ebs.arn  # Using AWS-managed encryption
    }
  }
...
# CloudWatch Alarm - High CPU (DISABLED - requires cloudwatch:PutMetricAlarm IAM permission)
...
# CloudWatch Alarm - Low CPU (DISABLED - requires cloudwatch:PutMetricAlarm IAM permission)
# Use Prometheus + Grafana for equivalent monitoring
